...
func (x *Rest) setupRouter() *httprouter.Router {
	...
	router.GET(`/category/:category`, x.Check(x.BasicAuth(x.CategoryShow)))
	router.GET(`/category/`, x.Check(x.BasicAuth(x.CategoryList)))
	...
	if !x.conf.ReadOnly {
		if !x.conf.Observer {
			...
			router.DELETE(`/category/:category`, x.Check(x.BasicAuth(x.CategoryRemove)))
			...
			router.POST(`/category/`, x.Check(x.BasicAuth(x.CategoryAdd)))
