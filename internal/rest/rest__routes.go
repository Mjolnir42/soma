...
func (x *Rest) setupRouter() *httprouter.Router {
	...
	router.GET(`/job/:jobID`, x.Authenticated(x.JobShow))
	router.GET(`/job/`, x.Authenticated(x.ScopeSelectJobList))
	...
	router.GET(`/oncall/:oncall`, x.Authenticated(x.OncallShow))
	...
	router.POST(`/search/job/`, x.Authenticated(x.JobSearch))
	...
	if !x.conf.ReadOnly {
		if !x.conf.Observer {
			...
			router.PATCH(`/oncall/:oncall`, x.Authenticated(x.OncallUpdate))
			...
			router.PUT(`/job/:jobID`, x.Authenticated(x.ScopeSelectJobWait))
