...
SELECT      soma.groups.group_id,
            soma.groups.group_name,
            soma.groups.bucket_id
FROM        inventory.user
...
WHERE       inventory.user.uid = $3::varchar
  ...
  AND       $2::varchar = 'list'
  AND       soma.groups.bucket_id = $4::uuid
UNION
...
func init() {
	m[AuthorizedGroupList] = `AuthorizedGroupList`
}
